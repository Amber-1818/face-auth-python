...
def get_embedding(face):
    # BETTER NORMALIZATION + RESIZE
    face = cv2.resize(face, (112, 112))  # BADA SIZE
    face = face.astype(np.float32) / 255.0
    face = (face - 0.5) / 0.5  # STRONG NORMALIZATION

    # 512-DIM EMBEDDING (BETTER ACCURACY)
    embedding = []
    for i in range(16):
        for j in range(16):
    ...
    embedding = embedding[:512]  # 512 DIM
